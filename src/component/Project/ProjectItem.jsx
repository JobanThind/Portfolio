...
import Camera_Opt from '../../assets/images/cam.gif';
import Stegnography from '../../assets/images/stegnographygif.gif';

const projectPhoto = {
    'Multi-User Chat Java Application': ChatApp,
    'Traffic Camera Optimization': Camera_Opt,
    'Image Stegnography': Stegnography
};
        ...
        {projectItems.map((item, index) => (
            <div key={index} className="flex flex-col md:flex-row items-center md:items-center mb-10 w-full">
                ...
                <div className="flex flex-col justify-between p-4 leading-normal w-full md:w-2/3">
                    ...
                    <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">Technologies Used: {item.technologies.join(' , ')}</p>
                </div>
            </div>
        ))}
    </div>
